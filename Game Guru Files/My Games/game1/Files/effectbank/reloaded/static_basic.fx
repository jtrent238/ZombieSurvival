...
float4 mainPS_highest(vertexOutput IN) : COLOR
{
    ...
    float4 flatlightingdiff = lighting - lit(dot(Ln,normalmap),dot(Hn,normalmap),24);

	// choose between surface lighting and lightmap based on shadow strength
	float4 rawlightmap = tex2D(OccSampler,IN.TexCoord2.xy);
	float4 omnilightmap = lerp(float4(0.5f,0.5f,0.5f,1),rawlightmap,ShadowStrength);
    float4 lightmap = lerp(float4(lighting.y*0.9f,lighting.y*0.9f,lighting.y*0.9f,1),rawlightmap*float4(1+flatlightingdiff.y,1+flatlightingdiff.y,1+flatlightingdiff.y,1),ShadowStrength);
	float4 fLightOfShadow = max(min(float4((lightmap.xyz-0.25f)*2,1),1),0); // 0.25=BLACK 0.75=WHITE -> 0-1 range
	...
	if ( ShadowStrength > 0.0f )
	{
		...
		fShadow = fShadow * ShadowStrength; // * IN.vDepth.y; 310315 - restore stronger entity shadows
	}
	
	// inverse to modulate surface lighting (and only shadow surfaces facing sunlight)
	fShadow = fShadow * dot(Ln,Nb) * 2.0f;
	fLightOfShadow = min(fLightOfShadow,1-fShadow);
    ...
    float4 ambContrib = (float4(bouncelightcolor,1) * ((AmbiColor * AmbiColorOverride)+float4(illummap.xxx,1))) * 4 * omnilightmap;
...
float4 mainPS_medium(vertexOutput_low IN) : COLOR
{
    ...
    float3 Vn  = normalize(V); 
    float3 Hn = normalize(Vn+Ln);
    float4 lighting = lit(pow(0.5*(dot(Ln,IN.WorldNormal))+0.5,2),dot(Hn,IN.WorldNormal),24);  //Valve's lighting model
	...
	float4 rawlightmap = tex2D(OccSampler,IN.TexCoord2.xy);
	float4 omnilightmap = lerp(float4(0.5f,0.5f,0.5f,1),rawlightmap,ShadowStrength);
    float4 lightmap = lerp(float4(lighting.y*0.9f,lighting.y*0.9f,lighting.y*0.9f,1),rawlightmap,ShadowStrength);
	float4 fLightOfShadow = max(min(float4((lightmap.xyz-0.25f)*2,1),1),0); // 0.25=BLACK 0.75=WHITE -> 0-1 range
	...
	float3 bouncelightcolor = lerp(FloorColor,SkyColor,fSkyFloorRatio) * diffusemap.xyz * 0.8;
	bouncelightcolor = bouncelightcolor + (diffusemap.xyz * 0.2);
    float4 ambContrib = float4(bouncelightcolor,1) * AmbiColor * AmbiColorOverride * 4 * omnilightmap;	
	float4 diffuseContrib = SurfColor * diffusemap * fLightOfShadow * GlobalSurfaceIntensity;
